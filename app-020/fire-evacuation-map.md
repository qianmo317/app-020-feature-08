...
4. **自动校验**（编辑后 500ms 防抖重算，不需要点按钮）：疏散最远距离、袋形走道死端、灭火器未覆盖面积、安全出口数量与连通性、房间无门、检查记录过期/缺失/损坏。结论直接标在图纸上：疏散最远点红圈 + 距最近出口距离、死端走道整段红色高亮 + 死端长度；多出口时可按出口查看各自服务区与最远服务距离（面板列表与图纸「显示出口分区」着色同色对应）；校验面板任意一条点击即在图上居中定位。
...
## 8. 关键算法
1. **走道栅格图 + Dijkstra**（`src/lib/graph.ts`）：把可行走多边形按 **0.25m** 栅格化（射线法掩码 + 按多边形 bbox 预过滤），再做 **1 格 4 邻膨胀**补上共边房间之间的断缝；安全出口节点按 **2.5m**、门节点按 **1.5m** 吸附到最近栅格点；8 邻连通，对角要求两个正交邻居都可行（防切角穿墙）；二叉堆多源 Dijkstra 得每个栅格点到最近出口的路径距离，并同步记录每个点的**最近出口**（出口服务分区：面板逐出口列最远服务距离，编辑器可按分区着色，房间按门的最近出口归属）。栅格数超过 8,000,000 直接抛 `floor too large for grid`。
...
4. **袋形走道死端**（`graph.ts` 的 `computeDeadEndDepth`）：多出口时 `depth(n) = min over 出口对 (i,j) of (d(n,i) + d(n,j) − D(i,j)) / 2`；单出口时 `depth(n) = d(n, 唯一出口)`，取全部栅格点的最大值。可用出口上限取 12 个。袋深最大点即死端尽端，从尽端沿袋深严格下降方向回溯到袋口，得到死端走道的中心线路径（图纸整段高亮用）；袋深 ≤3m 的端部小角落（出口背后的拐角）不上图，避免误报。
...
## 10. 验收标准
- 单元测试 **8 个文件 / 70 个用例**全部通过（vitest 2.1.9，`npm test`）：疏散距离 20 组、灭火器覆盖 10 组、检查台账 7 组、编号 6 组、store 回归 10 组、规则切换 4 组、性能 2 组、出口分区与定位 11 组。
